...
def solve(dist, sets, **params):
    ...
    Args:
        dist: square matrix of integer arc costs; ``dist[i][j]`` is the cost
            ...
            integer array is accepted.  Each entry must fit in 32 bits;
            scale and round floating-point costs before calling.
        sets: iterable of iterables of 0-indexed vertex ids; the sets must
            partition ``{0, ..., n-1}``.
        **params: solver options, all keyword-only: ``mode`` ("default",
            ...
            "insertion"), ``reopt``, ``epsilon``, ``noise``, ``budget``,
            ``seed`` (for reproducible runs), ``verbose`` (0-3), and
            ``output_file``.  Unset options use the mode's defaults, exactly
            as in the Julia implementation.
    ...
    sets = [[int(v) for v in s] for s in sets]
